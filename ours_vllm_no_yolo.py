...
SYSTEM_PROMPT = (
    "You are an expert train driver and railway safety inspector. This is a no-YOLO direct-VLM ablation: use only the raw image and do not assume any object detector annotations. "
...
def write_summary_files(summary, out_dir):
    json_path = os.path.join(out_dir, "ours_no_yolo_vllm_run_summary.json")
    md_path = os.path.join(out_dir, "ours_no_yolo_vllm_run_summary.md")
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Ours No-YOLO vLLM 运行汇总\n\n")
...
def main(args):
    ...
    resume_path = args.resume_file or os.path.join(args.out_dir, "ours_no_yolo_vllm_resume.jsonl")
    trace_path = args.trace_file or os.path.join(args.out_dir, "ours_no_yolo_vllm_trace.log")
    ...
    if not records_to_run:
        final_path = os.path.join(args.out_dir, f"ours_no_yolo_vllm_results_{time.strftime('%y%m%d_%H%M')}.json")
    ...
    print(f"Loading vLLM Qwen-VL for no-YOLO ablation from {args.model_path}...")
    llm = LLM(
    ...
    print(
        f"Starting vLLM Ours No-YOLO Inference: total={total}, "
        f"finished={start_completed}, remaining={len(records_to_run)}, batch_size={args.batch_size}"
    )
    start_time = time.perf_counter()
    progress = tqdm(total=total, initial=start_completed, unit="sample", desc="Ours No-YOLO vLLM", dynamic_ncols=True)
    ...
    out_path = os.path.join(args.out_dir, f"ours_no_yolo_vllm_results_{time.strftime('%y%m%d_%H%M')}.json")
    ...
    print(
        f"Ours No-YOLO vLLM finished: {len(outputs)}/{total} samples, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume-file", type=str, default=None, help="JSONL checkpoint file. Defaults to OUT_DIR/ours_no_yolo_vllm_resume.jsonl.")
    parser.add_argument("--trace-file", type=str, default=None, help="Readable input/output trace log. Defaults to OUT_DIR/ours_no_yolo_vllm_trace.log.")
